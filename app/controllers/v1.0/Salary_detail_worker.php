...
class Salary_detail_worker extends REST_Controller {
	...
	public function view_get(){
		try {
			...
			// lấy lương của thợ
			$data['worker'] = $this->worker($periodic, $id);

			// lấy công thợ từ công trình
			$accountant = $this->autoload_model->_get_where(array(
				'table' => 'accountant as tb1',
				'select' => '(SELECT fullname FROM user WHERE user.id = tb1.userid) as fullname, tb1.userid, tb1.money, tb1.constructionid', 
				'join' => array(
					array('construction as tb2' , 'tb1.constructionid = tb2.id AND tb2.trash = 0', 'left'),
				),
				'query' => 'tb1.trash = 0  AND  tb1.userid = '.$id.' AND tb2.date_start <= "'.$periodic['date_end'].'" AND tb2.date_start >= "'.$periodic['date_start'].'"',
			), true);
			// lấy ra danh sách công trình trong kì
			$construction = $this->autoload_model->_get_where(array(
				'table' => 'construction as tb1',
				'select' => 'tb1.date_start, tb1.fullname, tb1.phone, tb1.data_json, tb1.note, tb1.id', 
			...
			if(isset($construction) && check_array($construction)){
				foreach ($construction as $key => $val) {
					if(isset($accountant) && check_array($accountant)){
						// tính tổng thợ của từng công trình
						$total_money_worker = 0;
						foreach ($accountant as $sub => $subs) {
							$money = $subs['money'];
							$total_money_worker = $total_money_worker + $subs['money'];
							if($subs['constructionid'] == $val['id']){
								$construction[$key]['accountant'][] = $subs;
							}
						}
						if(isset($money) && $money == 0){
							unset($construction[$key]);
						}else{
							$construction[$key]['total_money_worker'] = $total_money_worker;
			...
			$cash = $this->autoload_model->_get_where(array(
				...
				'query' => 'tb1.trash = 0 AND tb1.userid = '.$id.' AND tb3.slug="tho" AND tb1.catalogueid=2 AND tb1.trash = 0 AND tb1.time <= "'.$periodic['date_end'].'" AND tb1.time >= "'.$periodic['date_start'].'"',
			), true);
	...
	public function worker($periodic = '', $id = ''){
		...
		$user = $this->autoload_model->_get_where(array(
			...
			'group_by' => 'tb1.id',
			'query' => 'tb1.trash = 0 AND tb1.id ='.$id.' AND tb3.slug="tho"',
		), true);
