...
class Product_catalogue extends REST_Controller {
	...
	protected function convert_data($data){
		...
		// $data['join'] = array(array('customer', 'customer.id = product_catalogue.customerid', 'left'));
		return $data;
	}
	private function process_select($field = ''){
		...
		if(strrpos($field, 'product_count')){
			$temp = '(SELECT COUNT(productid) FROM product_relationship WHERE product_relationship.catalogueid = '.$this->module.'.id AND product_relationship.module = "product" AND product_relationship.trash = 0) as product_count';
	...
	protected function config_panigation($data){
		return array(
			'start' => $data['start'], 
			'limit' => $data['limit'], 
			'base_url' => 'product/backend/catalogue/view'
		);
	}
	...
	public function delete_post($id = ''){
		...
		try {
			...
			$id = (int) $id;
			if($id <= 0) return $this->response(json_encode(array('code' => '404', 'result' => false, 'message' => 'Tham số truyền vào không hợp lệ')), 404);
			if(!check_permission($this->auth['id'] ?? '','product/backend/catalogue/delete')){
