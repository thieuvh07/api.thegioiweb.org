...
class common extends REST_Controller {
	...
	protected function convert_data($data){
		...
		// $data['join'] = array(array('customer', 'customer.id = common.customerid', 'left'));
		return $data;
	}
	...
	protected function set_data($data, $method){
		$field = array('customerid' , 'title', 'status', 'userid_charge', 'usercataid_charge');
	...
	protected function validate($data){
		...
		$this->form_validation->set_rules('customerid','Tên chủ thể','trim|required');
		$this->form_validation->set_rules('serviceid', 'Dịch vụ', 'trim|callback__check_default['.base64_encode(json_encode($data)).']');
	...
	function _check_default($str, $data){
		$data = json_decode(base64_decode($data), true);
		if(!isset($data['serviceid']) || $data['serviceid'] == '0' || $data['serviceid'] == ''){ 
			$this->form_validation->set_message('_check_default','Bạn chưa chọn dịch vụ');
     	 	return FALSE;
    	}
	}
	...
	protected function config_panigation($data){
		return array(
			'start' => $data['start'], 
			'limit' => $data['limit'], 
			'base_url' => 'common/backend/common/view'
		);
	}
