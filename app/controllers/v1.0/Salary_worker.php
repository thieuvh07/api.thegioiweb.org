...
class Salary_worker extends REST_Controller {
	...
	public function view_get(){
		try {
			...
			// lấy lương của thợ
			$data['worker'] = $this->worker($periodic);

			// lấy công thợ từ công trình
			$accountant = $this->autoload_model->_get_where(array(
				'table' => 'accountant as tb1',
				'select' => ' tb1.userid, tb1.money, tb1.constructionid,
					(SELECT fullname FROM user WHERE user.id = tb1.userid AND user.trash = 0) as fullname,', 
				'join' => array(
					array('construction as tb2' , 'tb1.constructionid = tb2.id AND tb2.trash = 0', 'left'),
				),
				'query' => 'tb1.trash = 0 AND tb2.date_start <= "'.$periodic['date_end'].'" AND tb2.date_start >= "'.$periodic['date_start'].'"',
			), true);
			...
			$cash = $this->autoload_model->_get_where(array(
				...
				'query' => 'tb1.trash = 0 AND tb3.slug="tho" AND tb1.catalogueid=2 AND tb1.trash = 0 AND tb1.time <= "'.$periodic['date_end'].'" AND tb1.time >= "'.$periodic['date_start'].'"',
			), true);
