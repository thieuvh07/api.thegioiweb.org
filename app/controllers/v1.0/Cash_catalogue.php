...
class Cash_catalogue extends REST_Controller {

	protected $module;
	protected $fieldKeywordArray;
	function __construct() {
		parent::__construct();
		$this->module = 'cash_catalogue';
	...
	protected function convert_data($data){
		...
		if(isset($queryList['trash'])){
			$query = $query.' AND trash = '.$queryList['trash'];
		}else{
			$query = $query.' AND trash = 0';
		}
		$data['query'] = substr($query, 4, strlen($query));
		$order_by = 'created DESC';
		$data['order_by'] = (!empty($queryData['order_by'])) ? $queryData['order_by'].', '.$order_by : $order_by;

		// $data['join'] = array(array('customer', 'customer.id = cash_catalogue.customerid', 'left'));
		return $data;
	}
	...
	protected function set_data($data, $method){
		$field = array('title', 'description', 'publish');
	...
	protected function validate($data){
		...
		$this->form_validation->set_rules('title','Tên nhóm tiền mặt','trim|required');
	...
	function _CheckCondition($title, $data){
		...
		if($slug != $slug_original){
			...
			if($count >= 1){
				$this->form_validation->set_message('_CheckCondition','Tên tiền mặt đã tồn tại');
				return false;
			}
			return true;
		}
	...
	protected function config_panigation($data){
		return array(
			'start' => $data['start'], 
			'limit' => $data['limit'], 
			'base_url' => 'cash/backend/catalogue/view'
		);
	}
	...
	public function delete_post($id = ''){
		...
		try {
			...
			$id = (int) $id;
			if($id <= 0) return $this->response(json_encode(array('code' => '404', 'result' => false, 'message' => 'Tham số truyền vào không hợp lệ')), 404);
			if(!check_permission($this->auth['id'] ?? '','cash/backend/catalogue/delete')){
